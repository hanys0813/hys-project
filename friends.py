...
pygame.display.set_caption("보스 처치 속성 각성 (연사력 상향 버전)")
...
def run_game():
    ...
    def show_boss_reward():
        nonlocal dmg, atk_spd, pierce_count, element, bullet_speed
        waiting = True
        while waiting:
            screen.fill(BLACK)
            ...
            draw_center("2. 전술적 가속 [공격력 0.7배 / 연사력 +320% / 관통+1]", 340, font_md, YELLOW)
            ...
            for ev in pygame.event.get():
                if ev.type == pygame.KEYDOWN:
                    ...
                    if ev.key == pygame.K_2: 
                        element = "Lightning"; dmg *= 0.7; atk_spd += 3.2; pierce_count += 1; bullet_speed = 22; waiting = False
    ...
    while running:
        ...
        if (keys[pygame.K_SPACE] or pygame.mouse.get_pressed()[0]) and shoot_tick <= 0:
            ...
            if targets:
                ...
                dist = math.hypot(dx, dy) or 1
                bullets.append({"pos": list(p_pos), "vel": [(dx/dist)*bullet_speed, (dy/dist)*bullet_speed], "p": pierce_count, "hit_list": []})
        ...
        if exp >= exp_next:
            ...
            # 연사력 증가 보너스 상향 (+0.35 -> +0.5)
            draw_center("1. 공격력 (+90)  2. 연사력 (+50%)  3. 체력 회복", 350, font_md, WHITE)
            pygame.display.flip()
            waiting = True
            while waiting:
                for ev in pygame.event.get():
                    if ev.type == pygame.KEYDOWN:
                        ...
                        if ev.key == pygame.K_2: atk_spd += 0.5; waiting = False
